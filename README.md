## Hey, I'm Jasha 👋🏾

**AWS Solutions Architect** building production cloud systems at the intersection of AI, FinTech, and real estate intelligence.

### 🏗️ Currently Building

**[Dubai Wealth AI](https://dubairealestateinvestor.com)** — Enterprise SaaS platform on AWS for high-net-worth real estate investors in the Dubai/GCC market.

`14 CDK Stacks` · `77 Lambda Functions` · `Bedrock AI + RAG` · `Aurora Serverless v2` · `Synthetics Canaries` · `Stripe Billing`

**What's under the hood:**
- 🏛️ 14 AWS CDK stacks (IaC — zero console clicks)
- ⚡ 77 Lambda functions on ARM64/Graviton2 behind API Gateway HTTP v2
- 🤖 Amazon Bedrock Knowledge Base with RAG pipeline (59K words of regulatory docs)
- 🔒 3-tier VPC across 3 AZs, KMS encryption, WAF, custom ES256 JWT auth
- 📊 CloudWatch Synthetics canaries + X-Ray distributed tracing
- 💳 Stripe subscription billing (4 tiers) + 13 investment calculators
- 🏠 1,122 live property listings across 106 Dubai neighborhoods

### ☁️ AWS Services I Use Daily

![Lambda](https://img.shields.io/badge/Lambda_(77)-FF9900?style=flat&logo=awslambda&logoColor=white)
![API Gateway](https://img.shields.io/badge/API_Gateway_v2-FF4F8B?style=flat&logo=amazonapigateway&logoColor=white)
![Aurora](https://img.shields.io/badge/Aurora_Serverless_v2-4053D6?style=flat&logo=amazonrds&logoColor=white)
![DynamoDB](https://img.shields.io/badge/DynamoDB-4053D6?style=flat&logo=amazondynamodb&logoColor=white)
![Bedrock](https://img.shields.io/badge/Bedrock_AI-232F3E?style=flat&logo=amazonaws&logoColor=white)
![CDK](https://img.shields.io/badge/CDK_(14_stacks)-232F3E?style=flat&logo=amazonaws&logoColor=white)
![CloudFront](https://img.shields.io/badge/CloudFront+WAF-8C4FFF?style=flat)
![Cognito](https://img.shields.io/badge/Cognito-DD344C?style=flat&logo=amazonaws&logoColor=white)
![ElastiCache](https://img.shields.io/badge/ElastiCache_Redis-DC382D?style=flat&logo=redis&logoColor=white)
![EventBridge](https://img.shields.io/badge/EventBridge-FF4F8B?style=flat)
![SES](https://img.shields.io/badge/SES_v2-232F3E?style=flat)
![Synthetics](https://img.shields.io/badge/Synthetics_Canaries-FF9900?style=flat)

### 🔧 Languages & Frameworks

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js_20-339933?style=flat&logo=nodedotjs&logoColor=white)
![React](https://img.shields.io/badge/React_18-61DAFB?style=flat&logo=react&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)

### 📈 Key Metrics

| Metric | Value |
|--------|-------|
| CDK Stacks | 14 (all deployed, production) |
| Lambda Functions | 77 (ARM64/Graviton2) |
| API Cost Savings | 71% (HTTP v2 vs REST) |
| Compute Cost Savings | 20% (Graviton2 vs x86) |
| DB Cost Savings | 60% (Aurora Serverless v2) |
| CloudFront Cache Hit | 87% |

### 📜 Certifications

- ✅ AWS Certified Cloud Practitioner (2025)
- 📚 AWS Solutions Architect Associate (In Progress)
- 📚 AWS Solutions Architect Professional (2026)

### 💼 Professional Background

- **Sotheby's International Realty** — Global Real Estate Advisor (luxury residential, HNWI clients)
- **Merrill Lynch** — Financial Services & Wealth Management
- **Former MLB Draft Pick** — Chicago Cubs organization
- **Founder & Solutions Architect** — Dubai Wealth AI

### 🤝 Open To

**Senior / Principal Solutions Architect** roles at Series B+ fintech, SaaS, and financial services companies. Remote or flexible. $200K+ base.

### 📫 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/jashabalcom)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:jashabalcom@gmail.com)
[![Portfolio](https://img.shields.io/badge/Live_Demo-dubairealestateinvestor.com-00C853?style=flat)](https://dubairealestateinvestor.com)
